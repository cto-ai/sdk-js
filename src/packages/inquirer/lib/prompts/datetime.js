...
class Datepicker extends Base {
  constructor(questions, rl, answers) {
    ...
    var standardizeTime = function(date) {
      return Date.parse('1/1/2000').set({
        hour: date.getHours(),
    ...
    this.opt.date.min =
      typeof this.opt.date.min === 'string'
        ? Date.parse(this.opt.date.min)
        : null;
    this.opt.date.max =
      typeof this.opt.date.max === 'string'
        ? Date.parse(this.opt.date.max)
        : null;
    this.opt.time.min =
      typeof this.opt.time.min === 'string'
        ? standardizeTime(Date.parse(this.opt.time.min))
        : null;
    this.opt.time.max =
      typeof this.opt.time.max === 'string'
        ? standardizeTime(Date.parse(this.opt.time.max))
        : null;
      ...
      switch (str) {
        ...
        case 'mmmm':
          selection.elements.push({
            ...
            set: val => {
              ...
              selection.date = validateDate(
                selection.date.set({ month: val - 1 })
              );
            }
          });
          break;
        case 'yy':
          selection.elements.push({
            ...
            set: val => {
              ...
              selection.date = validateDate(
                selection.date.set({ year: 2000 + val })
              );
            }
          });
          break;
        ...
        case 'HH':
          selection.elements.push({
            add: diff => {
              selection.date = validateDate(
                selection.date.addHours(diff * opt.time.hours.interval)
              );
            },
            set: val => {
              if (val >= 60) {
              ...
              if (selection.date.getHours() <= 12) {
                selection.date = validateDate(
                  selection.date.set({ hour: val })
                );
              } else {
                selection.date = validateDate(
                  selection.date.set({ hour: val + 12 })
                );
              }
            }
          });
        ...
        case 'MM':
          selection.elements.push({
            add: diff => {
              selection.date = validateDate(
                selection.date.addMinutes(diff * opt.time.minutes.interval)
              );
            },
            set: val => {
              if (val >= 60) {
        ...
        case 'ss':
          selection.elements.push({
            add: diff => {
              selection.date = validateDate(
                selection.date.addSeconds(diff * opt.time.seconds.interval)
              );
            },
            set: val => {
              if (val >= 60) {
  ...
  onKeypress(e) {
    ...
    if (e.key.name === 'right') {
      ...
      res = _.findLastIndex(
        selection.elements,
        isSelectable,
        selection.cur > 0 ? selection.cur - 1 : selection.cur
      );
      selection.cur = res >= 0 ? res : selection.cur;
